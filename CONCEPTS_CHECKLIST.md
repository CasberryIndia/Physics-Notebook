# Physics Concepts Checklist

This is a simplified checklist of all concepts, laws, and theories to be added to the Physics Notebook.

You can add the missing concept and domains to this list. So that anyone can use this checklist to create a new concepts to the notebook.

## Mechanics (Classical)
- [x] Newton's Laws of Motion
- [x] Conservation of Energy
- [x] Conservation of Momentum & Angular Momentum
- [x] Work-Energy Principle
- [x] Rotational Dynamics
- [x] Oscillations and Harmonic Motion
- [x] Lagrangian & Hamiltonian Mechanics
- [ ] Principle of Least Action
- [ ] Noether's Theorem
- [ ] Chaos and Nonlinear Dynamics

## Waves & Optics
- [ ] Wave Equation and Superposition Principle
- [ ] Interference
- [ ] Diffraction
- [ ] Polarization of Waves
- [ ] Geometrical Optics
- [ ] Physical Optics
- [ ] Lasers and Coherence
- [ ] Optical Instruments

## Electromagnetism
- [ ] Coulomb's Law
- [ ] Electric Fields & Potentials
- [ ] Gauss's Law
- [ ] Biot-Savart Law and Ampère's Law
- [ ] Faraday's Law of Induction
- [ ] Maxwell's Equations
- [ ] Lorentz Force Law
- [ ] Electromagnetic Waves
- [ ] Electric Circuits

## Thermodynamics & Statistical Mechanics
- [ ] Zeroth Law (Thermal Equilibrium)
- [ ] First Law (Conservation of Energy)
- [ ] Second Law (Entropy Increase)
- [ ] Third Law (Absolute Entropy)
- [ ] Entropy (Boltzmann's Equation)
- [ ] Thermodynamic Potentials
- [ ] Kinetic Theory of Gases
- [ ] Statistical Ensembles
- [ ] Phase Transitions
- [ ] Heat Engines & Carnot Cycle

## Quantum Mechanics
- [ ] Photoelectric Effect
- [ ] Blackbody Radiation
- [ ] Bohr Model of Atom
- [ ] de Broglie Waves
- [ ] Heisenberg Uncertainty Principle
- [ ] Schrödinger Equation
- [ ] Pauli Exclusion Principle
- [ ] Spin and Angular Momentum
- [ ] Quantum Statistics
- [ ] Quantum Field Theory

## Relativity
- [x] Special Relativity
- [ ] General Relativity
- [ ] Equivalence Principle
- [ ] Lorentz Transformation
- [ ] Gravitational Waves
- [ ] Cosmological Models

## Atomic, Nuclear & Particle Physics
- [ ] Atomic Structure
- [ ] Radioactivity
- [ ] Nuclear Physics
- [ ] Elementary Particles
- [ ] Standard Model

## Condensed Matter & AMO
- [ ] Crystal Lattices & Bragg Diffraction
- [ ] Band Theory of Solids
- [ ] Superconductivity
- [ ] Semiconductors & Transistors
- [ ] Quantum Hall Effect

## Astrophysics & Cosmology
- [ ] Stellar Structure & Evolution
- [ ] Cosmology (Big Bang, CMB)
- [ ] Black Holes & GR
- [ ] Dark Matter & Dark Energy

## Cross-Disciplinary & Methods
- [ ] Statistical & Computational Methods
- [ ] Mathematical Tools
- [ ] Instrumentation & Measurement
- [ ] Chaos & Complexity
